...
def request_login_token(email, cpf):
    """
    Gera e envia um token de login de 6 dígitos para o e-mail do paciente.
    """
    cpf_limpo = "".join(filter(str.isdigit, cpf))
    ...
    html_body = f"""
        ...
        <h2 style="font-size: 1.5em; letter-spacing: 3px; text-align: center; background-color: #f0f0f0; padding: 10px; border-radius: 5px;">
            <strong>{auth_token.token}</strong>
        </h2>
    """

    try:
        send_email(to=patient.email, subject="Seu Codigo de Acesso", template=html_body)
        return auth_token.token, "Token enviado para o seu e-mail."
...
def verify_login_token(token_str):
    """
    Verifica o token de 6 dígitos e retorna um token JWT se for válido.
    """
    try:
        ...
        if not secret:
            current_app.logger.error("ERRO - SECRET_KEY não está definido!")
        ...
        current_app.logger.error(f"ERRO INTERNO no verify_login_token: {e}")
