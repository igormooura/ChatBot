...
def handle_request_token():
    """Recebe email e cpf e solicita o envio de um token de acesso."""
    data = request.get_json()
...
def handle_verify_token():
    """Recebe um token de uso único e, se válido, retorna um token JWT de sessão."""
    data = request.get_json()
...
def get_my_profile(current_user):
    """Rota protegida de exemplo que retorna os dados do usuário logado."""
    if not current_user:
